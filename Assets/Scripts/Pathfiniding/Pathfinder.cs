...
public class Pathfinder 
{
            ...
            for (int yIndex = -1; yIndex <= 1; yIndex++)
            {
                for (int z = -1; z <= 1; z++)
                {
                    ...
                    else
                    {
                        ...
                        Node newNode = GetNeighboursNode(searchPos, true, node);
    ...
    // main search for a*
    // apply any tweaks here
    private Node GetNeighboursNode(Node adjPos, bool searchTopDown, Node currentNodePos)
    {
        Node retVal = null;
